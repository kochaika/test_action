...
RUN $QODANA_DIST/bin/idea.sh installCoursePlugins --marketplace $MARKETPLACE_COURSE_ID -Dplugin.path=/opt/plugins/JetBrainsAcademy:/opt/plugins/toml -Djava.awt.headless=true

ENTRYPOINT ["/opt/idea/bin/idea.sh", "validateCourse", "/project/data", "--marketplace=16630", "--tests=true", "--links=true", "-Dplugin.path=/opt/plugins/JetBrainsAcademy:/opt/plugins/toml", "-Djava.awt.headless=true"]
